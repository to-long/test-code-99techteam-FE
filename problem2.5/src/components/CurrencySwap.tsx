...
export const CurrencySwap = () => {
  ...
  const handleFromChange = useCallback((value: string) => {
    ...
    if (value && !isNaN(parseFloat(value)) && exchangeRate) {
        const computed = (parseFloat(value) * exchangeRate).toFixed(6);
        // Remove trailing zeros if needed, or keep precision
        setToAmount(parseFloat(computed).toString());
    } else {
        setToAmount('');
    }
  ...
  const handleToChange = useCallback((value: string) => {
    ...
    if (value && !isNaN(parseFloat(value)) && exchangeRate) {
        const computed = (parseFloat(value) / exchangeRate).toFixed(6);
        setFromAmount(parseFloat(computed).toString());
        
        // Update form validation with the COMPUTED from amount
        setValue('amount', parseFloat(computed), { shouldValidate: true });
    } else {
        setFromAmount('');
                ...
                <div className="h-6 mt-4">
                    {fromAmount && !isNaN(parseFloat(fromAmount)) && (
                        <div className="text-sm text-gray-500 font-medium font-mono">
                            ${(parseFloat(fromAmount) * (tokens.find(t => t.currency === fromCurrency)?.price || 0)).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </div>
                    )}
                </div>
            ...
            >
                ...
                <div className="h-6 mt-4">
                    {toAmount && !isNaN(parseFloat(toAmount)) && (
                        <div className="text-sm text-gray-500 font-medium font-mono">
                            ${(parseFloat(toAmount) * (tokens.find(t => t.currency === toCurrency)?.price || 0)).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </div>
                    )}
                </div>
            </div>
